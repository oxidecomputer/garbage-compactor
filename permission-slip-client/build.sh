...
RNAM='permission-slip'
NAM="$RNAM-client"
#
# permission-slip is a private repository, so we use the ssh URL:
#
REPO="ssh://git@github.com/oxidecomputer/$RNAM.git"
...
VER=$(cargo metadata --format-version 1 |
    jq -r ".packages | map(select(.name == \"$NAM\")) | .[].version"
    ).$commit_count
