...
NAM='permission-slip-client'
# permission slip is private so we use the ssh URL
REPO="ssh://git@github.com/oxidecomputer/permission-slip.git"
...
VER=$(cargo metadata --format-version 1 |
    jq -r '.packages | map(select(.name == "permission-slip-client")) | .[].version'
    ).$commit_count
...
info "build $NAM..."
cargo build --bin permslip --release --locked

header "installing $NAM"
rm -rf "$PROTO"
mkdir -p "$PROTO/usr/bin"
for cmd in permslip; do
...
ips)
	make_package "developer/debug/$NAM" \
	    'device identity certificate support tools' \
	    "$WORK/proto"
	header 'build output:'
