...
#
# Determine the version embedded in the built binary. Not every commit
...
VER=$(cargo metadata --format-version 1 |
    jq -r '.packages | map(select(.name == "thundermuffin")) | .[].version'
    ).$commit_count
...
ips)
	make_package "network/$NAM" \
	    'thundermuffin: a network throughput testing tool' \
	    "$WORK/proto"
	header 'build output:'
