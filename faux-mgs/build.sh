...
NAM='faux-mgs'
REPO="https://github.com/oxidecomputer/management-gateway-service.git"
...
VER=$(cargo metadata --format-version 1 |
    jq -r '.packages | map(select(.name == "faux-mgs")) | .[].version'
    ).$commit_count
...
cp "target/release/$NAM" "$PROTO/usr/bin/$NAM"
...
case "$OUTPUT_TYPE" in
ips)
	make_package "network/$NAM" \
	    'Client tool for management network access to Oxide SP' \
	    "$WORK/proto"
	header 'build output:'
